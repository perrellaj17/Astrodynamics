...
re = 6378.1363; %mean equitorial radius of the Earth
...
A = 0.0001; %Exposed cross sectional area in km (10cm)
BS = msat/(Cd*A); %Ballistic coefficient
...
opts = odeset('RelTol',1e-6,'AbsTol',1e-10);
[t1,r1] = ode45(@Syssim2,[0 tf],R1,opts,mu);
...
%     
    ...
    %     %then we create a vector that is perpendicular to mdHAT and in the same
    %     %direction as sat. We will call that perp
    %
    %     perpx(i) = satx(i);
    %     perpy(i) = saty(i);
    %     perpz(i) = (-mdx(i)*satx(i) - mdy(i)*saty(i))/mdz(i);
    %
    %     perpmag(i) = sqrt(perpx(i)^2 + perpy(i)^2 + perpz(i)^2);
    %
    %from there we can compute the dot product of sat and perp vectors and see what the
    %angle is between them. This should give us the elevation from the surface
    %of the earth to the satellite from the current md location.
...
% earth.FaceColor = 'b';
% earth.EdgeColor = 'g';
% earth.FaceAlpha = 0.15;
% earth.EdgeAlpha = 0.15;
% 
% 
% 
